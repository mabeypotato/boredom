...
%      k_I = ; % frequency at which integral action ends
...
%     Lead =tf((sqrt(M)+M)/(sqrt(M)+1)*[1,wmax/sqrt(M)],[1,wmax*sqrt(M)]);
%     C = C*Lead;
%     %figure(1), margin(Plant*C), figure(2), bodemag(Plant*C) % update plot
...
figure(7), clf
    ...
    subplot(3,1,3), 
        stepsize = 10*pi/180;
        step(stepsize*CLOSED_R_to_U), hold on
        step(stepsize*CLOSED_R_to_U*F)
...
[P.A_C,P.B_C,P.C_C,P.D_C]=tf2ss(P.num_C, P.den_C);
[P.A_F, P.B_F, P.C_F, P.D_F] = tf2ss(P.num_F,P.den_F);
