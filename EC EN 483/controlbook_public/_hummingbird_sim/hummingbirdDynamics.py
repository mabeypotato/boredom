...
class HummingbirdDynamics:
    def __init__(self, alpha=0.0):
        ...
        self.km = P.Fe * (1. + alpha * (2. * np.random.rand() - 1.))
    ...
    def _M(self, state: np.ndarray):
        ...
        M33 = ((self.m1*self.ell1**2 + self.m2*self.ell2**2 + self.J2z + self.J1y*np.sin(phi)**2 + self.J1z*np.cos(phi)**2)
               *np.cos(theta)**2 + (self.J1x + self.J2x)*np.sin(theta)**2 + self.m3*(self.ell3x**2 + self.ell3y**2) + self.J3z)

        # Return the M matrix
        return np.array([[self.J1x, 0, -1*self.J1x*np.sin(theta)],
                      [0,                 M22,               M23],
                      [-1*self.J1x*np.sin(theta),   M23 ,    M33]])
    ...
    def _C(self, state: np.ndarray):
        ...
        N33 = (2*(self.J1x + self.J2x - self.m1*self.ell1**2 - self.m2*self.ell2**2 - self.J2z - self.J1y*np.sin(phi)**2
                  - self.J1z*np.cos(phi)**2)*np.sin(theta)*np.cos(theta)            
        )

        C_1 = ((self.J1y - self.J1z)*np.sin(phi)*np.cos(phi)*(thetadot**2 - np.cos(theta)**2*psidot**2) + ((self.J1y - self.J1z)*(np.cos(phi)**2 
                - np.sin(phi)**2) - self.J1x)*np.cos(theta)*thetadot*psidot
            )


        C_2 = (2*(self.J1z - self.J1y)*np.sin(phi)*np.cos(phi)*phidot*thetadot + ((self.J1y - self.J1z)*(np.cos(phi)**2 
                - np.sin(phi)**2) + self.J1x)*np.cos(theta)*phidot*psidot - 1/2*N33*psidot**2
        )

        C_3 = (thetadot**2*(self.J1z - self.J1y)*np.sin(phi)*np.cos(phi)*np.sin(theta) + np.cos(theta)*phidot*thetadot*
               ((self.J1y - self.J1z)*(np.cos(phi)**2 - np.sin(phi)**2) - self.J1x) + np.sin(phi)*np.cos(phi)*np.sin(theta)*
               thetadot**2*(self.J1z - self.J1y) + 2*(self.J1y - self.J1z)*np.sin(phi)*np.cos(phi)*phidot*psidot + 2*(-1*self.m1*
                self.ell1**2 - self.m2*self.ell2**2 - self.J2z + self.J1x + self.J2x + self.J1y*np.sin(phi)**2 + self.J1z*(np.sin(phi)**2))
    ...
    def _partialP(self, state: np.ndarray):
        ...
        partialP = (self.m1*self.ell1 + self.m2*self.ell2)*P.g*np.cos(theta)
    ...
    def _tau(self, state: np.ndarray, force: float, torque: float):
        ...
        tau_1 = torque
        tau_2 = self.ellT*(force)*np.cos(phi)
        tau_3 = self.ellT*(force)*np.cos(theta)*np.sin(phi) - torque*np.sin(theta)
        # Return the tau matrix
        return np.array([[tau_1],
                        [tau_2],
                        [tau_3]])
